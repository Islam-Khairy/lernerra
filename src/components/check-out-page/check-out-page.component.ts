...
import { StripeCardGroupDirective, StripeCardCvcComponent, StripeCardExpiryComponent } from "ngx-stripe";
...
@Component({
  ...
  imports: [StripeCardNumberComponent, StripeCardGroupDirective, StripeCardCvcComponent, StripeCardExpiryComponent, ToastModule],
...
pay() {
  ...
  this.payment.checkout({courseID:this.courseId}).subscribe(
    ...
    if(cardElement)
      {
      const result = this.StripeService.confirmCardPayment(this.clientSecret, {
        payment_method: {
          card: cardElement,
          billing_details: {
      ...
      }).subscribe(
        {
          next: async (paymentResult) => {
                ...
                if (paymentResult.error) {
                  ...
                  this.showMessage("Payment failed, Please try again later.",'error');
...
catch (err) 
{
      ...
      console.error(err);
      this.showMessage('An error occurred during payment. try again later','error');
}
