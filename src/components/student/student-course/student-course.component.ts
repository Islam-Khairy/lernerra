...
export class StudentCourseComponent {
  ...
  constructor(
    ...
    private courseProgressService: CoursePrograssService,
  ...
  getCourseLessons() {
    this.courseService.getCourseById(parseInt(this.courseId())).subscribe({
      next: (res) => {
        ...
        this.vedioURL.set(this.course()?.lessons[0]?.vedioURL || '');
      },
      error: (err) => {
        console.log(err);
      },
  ...
  markLessonAsCompleted(lessonId: number) {
    this.courseProgressService
      .markLessonAsCompleted(lessonId, this.userId)
      .subscribe({
        next: (res) => {
  ...
  getCourseProgress() {
    console.log(this.courseId());
    this.courseProgressService
      .getCoursePrograss(parseInt(this.courseId()))
      .subscribe({
        next: (res) => {
          console.log(res);
  ...
  addRating() {
    this.courseService
      .addRating(parseInt(this.courseId()), this.rating)
      .subscribe({
        next: (res) => {
          console.log(res);
        ...
        error: (err) => {
          console.log(err);
          this.toastr.error("you have already rated this course", 'Error', {
            progressBar: true,
            progressAnimation: 'increasing',
            closeButton: true,
          });
